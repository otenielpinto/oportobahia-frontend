...
- `MONGO_CONNECTION` — MongoDB connection string for app data
- `MONGO_DATABASE` — MongoDB database name for app data
- `NEXT_AUTH_SECRET` — JWT signing secret (generate with `openssl rand -base64 32`)
- `NEXT_AUTH_MONGO_URL` — MongoDB connection for auth (separate from app data)
- `NEXT_AUTH_MONGO_DATABASE` — Auth database name

Copy `.env.example` to `.env` and fill values before running dev.

## Architecture

### Two MongoDB Connections

This project uses **two separate MongoDB connections**:

1. **App data** (`src/infra/mongoClient.ts`) — accessed via `TMongo.connectToDatabase()`
2. **Auth** (`src/auth/infra/mongoClient.ts`) — accessed via `TAuthMongo.connectToDatabase()`

Do NOT mix these. Auth operations use `TAuthMongo`, all other data uses `TMongo`.

### Custom JWT Auth (NOT NextAuth)

Authentication is custom-built with `jose` library:
- Session stored as JWT cookie (`session`)
- 1-day expiration
- No NextAuth/NextAuth.js used

Get current user in server actions: `import { getUser } from "@/actions/sessionAction"`

### Tenant-Aware Queries

All data queries filter by `id_tenant` and `id_empresa` from session:
```ts
...
- `(auth)` — sign-in, sign-up, logout (no sidebar)
- `(private)` — authenticated pages with sidebar layout

### Server Actions Pattern

All data mutations use `"use server"` actions:
- Start: `await TMongo.connectToDatabase()`
- Check auth: `await getUser()` (throw if null)
- Query with tenant filters
- End: `await TMongo.mongoDisconnect(client)`
- Revalidate: `revalidatePath("/route")`

### MongoDB Data Serialization

MongoDB returns objects with `_id` as ObjectId. Use `serializeMongoData()` from `src/lib/serializeMongoData.ts` before returning to client.
...
Pattern: react-hook-form + zod schema + @hookform/resolvers:
```ts
...
Configured with 10-minute staleTime, no refetch on window focus. Wrap queries with `useQuery` or server action hooks from `src/hooks/server-action-hooks.ts`.

## Language

Brazilian Portuguese (`lang="pt-BR"`). UI labels and messages in Portuguese.